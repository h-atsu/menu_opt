...
columns = ['price', 'energy', 'protein', 'fat', 'carbohydrates', 'salt', 'calcium', 'vegetable']
...
for link in all_menu_links:    
    ...
    for i, col in enumerate(columns):
        menu_dic[f'{col}'] = menu_detail_list[i].find('span', class_='price').text
...
f1 = lambda x : float(x[:-1])
f2 = lambda x : float(x[:-2])
f4 = lambda x : float(x[:-4])
